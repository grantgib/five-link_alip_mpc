...
for i=1:2:length(gait)
    ...
    free = [0 0 ones(1,bez_degree-5) 0 0];
    ...
    for k = 1:length(t_actual)
        ...
        traj{i}.hd_check(:,k) = full(f_ha([traj{i}.q(:,k); traj{i}.q_dot(:,k)]));
        
        vel_swing_k = full(f_vel_swing(traj{i}.q(:,k), traj{i}.q_dot(:,k)));
        traj{i}.hd_dot(:,k) = full(f_ha_dot([traj{i}.q(:,k); traj{i}.q_dot(:,k)]));
