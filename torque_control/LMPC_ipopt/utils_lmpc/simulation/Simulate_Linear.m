...
f_s_time = ref_info.time_based.f_s_time;
...
% foot positions
pos_sw_init = full(f_pos_swing(x_init(1:7)))';
pos_sw_traj = pos_sw_init;
...
while(traj_info.num_impacts < num_steps && ctrl_info.iter < num_steps*size(X_REF_Original,2))
    ...
    args.x0  = [reshape(delta_X_guess(:,1:N+1),n_x*(N+1),1);
                reshape(delta_U_guess(:,1:N),n_u*(N),1);
                reshape(delta_W_guess(:,1:N),n_w*(N),1)];
    ...
    params = struct('pos_sw_traj', pos_sw_traj);
    ...
    y_sw_next = full(f_pos_swing(x_next(1:7)))';
    y_st_next = full(f_pos_stance(x_next(1:7)))';
    ...
    pos_sw_traj = [pos_sw_traj, y_sw_next];
    pos_st_traj = [pos_st_traj, y_st_next];
...
traj_info.y_sw = pos_sw_traj;
traj_info.y_st = pos_st_traj;
