...
if SAVE_SOLUTION
    ...
    save_dir = fullfile(cur,'../../reference_trajectories/time_based_LIP_virts/');
...
%% Plots
% Extract Solution
q_traj = gait(1).states.x;
dq_traj = gait(1).states.dx;
u_traj = gait(1).inputs.u;
...
for i = 1:7
    subplot(3,3,i);
    plot(gait(1).tspan,q_traj(i,:))
    title("q_" + i);
end
figure
for i = 1:7
    subplot(3,3,i);
    plot(gait(1).tspan,dq_traj(i,:))
    title("dq_"+i);
end
...
alpha_virt = reshape(params{1}.atime,4,8);
delta_time = 0.005;
...
figure; 
virt_headers={'Torso Angle','Stance To COM Z','Swing to COM X','Swing to COM Z'};
for j = 1:4
    subplot(1,4,j);
    hold on;
    plot(tdes,q_virt(j,:));
