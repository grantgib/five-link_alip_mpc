...
y_bound_idx = str_indices({'torso_angle','pos_st_TO_com_z','pos_sw_TO_com_x','pos_sw_TO_com_z'},y_pos.OutputLabel);
...
if isequal(swingxmid,"free")
    swingxmid = 0;
end
swing_foot_height = SymFunction('swing_foot_height', nlp.Plant.EventFuncs.leftFootHeight.ConstrExpr, {X});
addNodeConstraint(nlp, swing_foot_height, {'x'}, floor(nlp.NumNode/2), swingxmid, Inf,'Linear');

% Swing Foot Length
if isequal(step_length,"free")
    step_length = Inf;
end
pos_swing_x = SymFunction('pos_swing_x',p_swing(1),{X});
addNodeConstraint(nlp,pos_swing_x,{'x'},'last',0.05,step_length,'Linear');

% foot x impact velocity
if isequal(swingxvel,"free")
    swingxvel = Inf;
end
