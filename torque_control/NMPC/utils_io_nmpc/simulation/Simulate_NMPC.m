function [traj_info,ctrl_info] = ...
    Simulate_NMPC(dyn_info,ctrl_info,ref_info,constr_info)
import casadi.*
...
f_s_phase = ref_info.phase_based.f_s_phase;

pos_sw_start = full(f_pos_swing(x_init(1:7)))';
...
w_ref_traj = [];
u_ref_traj = [];
s_traj = full(f_s_phase(x_init(1:n_q)));
impact_traj = [];
...
while(traj_info.num_impacts < num_steps && ctrl_info.iter < num_steps*size(X_REF_Original,2))
    ...
    x_traj_all(:,:,ctrl_info.iter) = reshape(full(sol.x(1:n_x*(N+1)))',n_x,N+1);     u_traj_all(:,:,ctrl_info.iter) = reshape(full(sol.x(n_x*(N+1)+1:(n_x+n_u)*(N+1)))',n_u,N+1);
...
%             zeros(12,size(X_REF_FULL,2))];
        ...
        pos_sw_start = full(f_pos_swing(x_next(1:7)))';
        pos_sw_current = pos_sw_start;
        pos_sw = [pos_sw, pos_sw_current];
        pos_st = [pos_st, pos_st_current];
        pos_sw_offset = [pos_sw_start(1)*(traj_info.num_impacts);
            pos_sw_start(2)*traj_info.num_impacts];
        pos_sw_normal = [pos_sw_normal, (pos_sw_current - pos_sw_offset)];
    else
        % Update output trajectory
        pos_sw = [pos_sw, pos_sw_current];
        pos_st = [pos_st, pos_st_current];
        pos_sw_offset = [pos_sw_start(1)*(traj_info.num_impacts+1);
            pos_sw_start(2)*(traj_info.num_impacts+1)];
        pos_sw_normal = [pos_sw_normal, (pos_sw_current-pos_sw_offset)] ;
    end
    % Update s phase variable
    s_traj = [s_traj, full(f_s_phase(x_next(1:n_q)))];
