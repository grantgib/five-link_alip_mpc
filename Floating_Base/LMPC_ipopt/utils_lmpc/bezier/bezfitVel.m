function [alpha err] = bezfitVel(s,ds, X,dX, arg3, afixed, W)
    ...
    if isscalar(arg3)
        ...
        if nargin<4 || isempty(afixed)
            afixed = getfixed(s, ds,X,dX, M, free);
        end
    end
    
    P = pascal(M+1,1);
    Q = diag((-1).^(0:M))*P*diag((1).^(0:M));
    B = fliplr(diag(ExoCalculations.binom(M)));
...
function afixed = getfixed(s,ds, X,dX, M, free)
    if any(~free(3:end-2))
