...
delta_w_header = {'$\delta \lambda_x$','$\delta \lambda_z$'};
...
if plotSettings.u
    ...
    for i = 1:n_u
        ...
        hold on; plot(time_traj(1:end-1),u_traj(i,1:size(time_traj(1:end-1),2)),'LineWidth',width_traj,'LineStyle',':');
...
%% delta_w
if plotSettings.delta_w
    figure
    for i = 1:n_w
        subplot(1,2,i);
        hold on;
        plot(time_traj(1:end-1),delta_w_traj(i,1:size(time_traj(1:end-1),2)),'LineWidth',width_traj);
        title(delta_w_header{i},'interpreter','latex');
        grid on; set(gca,'FontSize',sz);
    end
    legend("LMPC Wrench",'location','best');
    sgtitle("Delta Wrench");
    set(gcf,'color','w');
end
