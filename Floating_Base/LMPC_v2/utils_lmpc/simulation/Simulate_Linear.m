...
fe_linear = dyn_info.func.fe_linear;
...
while(traj_info.num_impacts < num_steps && ctrl_info.iter < num_steps*size(X_REF_Original,2))
    ...
    [P,c,Aeq,beq,A,B] = Build_QP_Matrices(dyn_info,ctrl_info,ref_info,delta_x_init,X_REF,U_REF,W_REF,impact_occurred,y_st_traj(:,end));
    ...
    %% Extract solution
    delta_xe_sol_mpc = reshape(sol(1:(N+1)*n_xe,1),n_xe,N+1);
    delta_p_sol_mpc = [delta_xe_sol_mpc(n_q+1:n_q+n_p,:); delta_xe_sol_mpc(n_qe+n_q+1:end,:)];
    delta_x_sol_mpc = [delta_xe_sol_mpc(1:n_q,:); delta_xe_sol_mpc(n_qe+1:n_qe+n_q,:)];
    delta_u_sol_mpc = reshape(sol((N+1)*n_xe+1:(N+1)*n_xe+N*n_u,1),n_u,N);
    delta_w_sol_mpc = reshape(sol((N+1)*n_xe+N*n_u+1:end,1),n_w,N);
    ...
    %     xe,u,w,delta_xe,delta_u,delta_w
    YST_REF = repmat(y_st_traj(:,end),1,N+1);
    XE_REF = [X_REF(1:n_q,1:N+1); YST_REF; X_REF(n_q+1:end,1:N+1); zeros(n_p,N+1)];
    
    delta_xe_sol_mpc
    for i = 1:N+1
%         disp("k = " + (i-1));
%         delta_xe = delta_xe_sol_mpc(:,i);
%                 disp("delta_xe = ");
%         delta_xe
        if i <= N
            delta_xedot = full(fe_linear(XE_REF(:,i),U_REF(:,i),W_REF(:,i),delta_xe_sol_mpc(:,i),delta_u_sol_mpc(:,i),delta_w_sol_mpc(:,i)));
            disp("dp = ");
            delta_dp = delta_xedot(n_q+1:n_qe)
            disp("ddp = ");
            delta_ddp = delta_xedot(n_qe+n_q+1:end)
            disp("delta_xedot = ");
            delta_xedot
        end
    end
    disp("******************* ITERATION # " + ctrl_info.iter + " *************************");
    ...
    for i = 1:N
        ...
        %         disp("iteration = " + ctrl_info.iter + " |k = " + i + ": Jc*ddq + Jc_dot*dq = ");
        %         disp(contact_value);
    end
