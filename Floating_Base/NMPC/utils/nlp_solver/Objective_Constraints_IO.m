...
for k = 1:N+1
    ...
    if ctrl_info.IO_info.linear == true
        u_IO_k = dyn_info.func.u_IO(q_k,dq_k,h_d_k,dh_d_k,ddh_d_k,Kp,Kd);      
    elseif ctrl_info.IO_info.linear == false
        u_IO_k = dyn_info.func.u_IO_NMPC(q_k,dq_k,h_d_k,dh_d_k,ddh_d_k,Kp,Kd,v_NL_k);
%         u_IO_k = dyn_info.func.u_IO_NMPC(q_k,dq_k,h_d_k,dh_d_k,ddh_d_k,v_NL_k);
        % notice that Kp,Kd are replaced with desired NMPC control value
        % (v_NL_k)
    end
    u_k = u_IO_k;
