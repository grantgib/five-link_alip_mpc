function right_stance_constraints(nlp, bounds, varargin)
    ...
    y_bound_idx = str_indices({'torso_angle','stance_knee_angle','stance_leg_angle','swing_leg_angle'},y_pos.OutputLabel);
    ...
    addNodeConstraint(nlp, swingFootHeight, {'x'}, floor(nlp.NumNode/2), 0.1*1.2, Inf,'Linear');
    
    %% Step Length
%     p_swing = getCartesianPosition(domain, domain.ContactPoints.LeftToe);
%     swingFootLength = SymFunction('swing_foot_length',p_swing(3),{domain.States.x});
%     addNodeConstraint(nlp,swingFootLength,{'x'},'last',0.2,Inf,'Linear');
